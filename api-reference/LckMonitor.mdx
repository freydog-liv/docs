...
`LckMonitor` is a component implementing [`ILckMonitor`](/api-reference/ILckMonitor) that represents a display surface (monitor) in the LCK.\
It manages a unique monitor ID for registration with the [`LckCamera`](/api-reference/LckCamera) and provides a RenderTexture output for connected [`LckCamera`](/api-reference/LckCamera) instances.
